...
<head>
    ...
    <title>BolaNow - Livescore Sepak Bola Terkini</title>
    <meta name="description" content="Dapatkan update skor pertandingan sepak bola secara real-time dari seluruh dunia">
    <meta name="keywords" content="livescore, sepak bola, skor langsung, jadwal pertandingan, hasil pertandingan">
...
<body class="bg-gray-100 text-gray-800">
    ...
    <main class="container mx-auto px-4 py-6">
        ...
        <div class="flex justify-between items-center mb-6 overflow-x-auto whitespace-nowrap py-2">
            <a href="?date=<?= date('Y-m-d', strtotime('-2 days')) ?>" class="px-3 py-1 rounded-full hover:bg-gray-200">
                <?= date('D, d M', strtotime('-2 days')) ?>
            </a>
            <a href="?date=<?= date('Y-m-d', strtotime('-1 day')) ?>" class="px-3 py-1 rounded-full hover:bg-gray-200">
                <?= date('D, d M', strtotime('-1 day')) ?>
            </a>
            <a href="?date=<?= date('Y-m-d') ?>" class="px-3 py-1 rounded-full bg-green-700 text-white">
                Hari Ini
            </a>
            <a href="?date=<?= date('Y-m-d', strtotime('+1 day')) ?>" class="px-3 py-1 rounded-full hover:bg-gray-200">
                <?= date('D, d M', strtotime('+1 day')) ?>
        ...
        <!-- Livescores -->
        <div class="space-y-6">
            <?php if (isset($livescores['data']) && !empty($livescores['data'])): ?>
                <?php
                    ...
                    <div class="bg-white rounded-lg shadow-md overflow-hidden">
                        ...
                        <div class="divide-y">
                            <?php foreach ($league_data['matches'] as $match): ?>
                                <div class="p-3 hover:bg-gray-50 transition-colors match-row" data-match-id="<?= $match['id'] ?>">
                                    <div class="flex justify-between items-center">
                                        <!-- Match Status -->
                                        <div class="w-16 text-center">
                                            <?php if ($match['status'] == 1): ?>
                                                <span class="text-red-600 font-medium"><?= $match['time']['minute'] ?>'</span>
                                            <?php elseif ($match['status'] == 0): ?>
                                                <span class="text-gray-600"><?= date('H:i', strtotime($match['time']['time'])) ?></span>
                                            <?php elseif ($match['status'] == 3): ?>
                                                <span class="text-gray-600">Selesai</span>
                                            <?php else: ?>
                                                <span class="text-gray-600"><?= $match['status_name'] ?></span>
                                            <?php endif; ?>
                                        </div>
                                        ...
                                        <div class="flex-1 grid grid-cols-7 gap-2 items-center">
                                            ...
                                            <div class="col-span-1 text-center">
                                                ...
                                                <?php else: ?>
                                                    <div class="font-bold">
                                                        <span class="score-home"><?= $match['scores']['home_score'] ?></span>
                                                        -
                                                        <span class="score-away"><?= $match['scores']['away_score'] ?></span>
                                                    </div>
                                                    <?php if (!empty($match['scores']['ht_score'])): ?>
                                                        <div class="text-xs text-gray-500">HT: <?= $match['scores']['ht_score'] ?></div>
                                                    <?php endif; ?>
                                                <?php endif; ?>
                                            </div>
            ...
            <?php else: ?>
                <div class="bg-white rounded-lg shadow-md p-6 text-center">
                    <p class="text-gray-500">Tidak ada pertandingan yang berlangsung saat ini.</p>
                </div>
            <?php endif; ?>
        </div>
    </main>
    ...
    <script>
        ...
        // Auto-refresh livescores every 60 seconds
        setInterval(function() {
            fetch('api/livescores.php')
                .then(response => response.json())
                .then(data => {
                    if (data.success) {
                        updateScores(data.matches);
                    }
                })
                .catch(error => console.error('Error fetching livescores:', error));
        }, 60000);

        // Update scores with visual indication
        function updateScores(matches) {
            matches.forEach(match => {
                const matchRow = document.querySelector(`.match-row[data-match-id="${match.id}"]`);
                if (matchRow) {
                    const homeScoreEl = matchRow.querySelector('.score-home');
                    const awayScoreEl = matchRow.querySelector('.score-away');
                    
                    if (homeScoreEl && awayScoreEl) {
                        const currentHomeScore = homeScoreEl.textContent;
                        const currentAwayScore = awayScoreEl.textContent;
                        
                        if (currentHomeScore !== match.home_score || currentAwayScore !== match.away_score) {
                            homeScoreEl.textContent = match.home_score;
                            awayScoreEl.textContent = match.away_score;
                            
                            // Flash effect for score change
                            homeScoreEl.parentElement.classList.add('score-flash');
                            setTimeout(() => {
                                homeScoreEl.parentElement.classList.remove('score-flash');
                            }, 1000);
                        }
                    }
                }
